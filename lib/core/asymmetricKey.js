...
export async function generateKey({keyId, type, controller} = {}) {
  ...
  if(type.includes('Ed25519')) {
    ...
    keyPair = await MldsaMultikey.generate({id: keyId, algorithm: 'ML-DSA-44'});
  } else if(type === 'urn:webkms:multikey:ML-DSA-65') {
    keyPair = await MldsaMultikey.generate({id: keyId, algorithm: 'ML-DSA-65'});
  } else if(type === 'urn:webkms:multikey:ML-DSA-87') {
    keyPair = await MldsaMultikey.generate({id: keyId, algorithm: 'ML-DSA-87'});
  } else {
